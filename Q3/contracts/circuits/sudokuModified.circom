...
template sudoku() {
    ...
    // initializing two 9x9 arrays of RangeProofs for the puzzle and solution matrices
    component puzzleRangeMatrix[9][9];
    component solutionRangeMatrix[9][9];
    // iterating through all elements of the 9x9 matrices
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            // defining the current element of the matrix as a RangeProof and setting the upper and lower bound of the range
            // we do this for both the puzzle and solution RangeProof matrices
            puzzleRangeMatrix[i][j] = RangeProof(32);
            puzzleRangeMatrix[i][j].range[0] <== 0;
            puzzleRangeMatrix[i][j].range[1] <== 9;
            solutionRangeMatrix[i][j] = RangeProof(32);
            solutionRangeMatrix[i][j].range[0] <== 0;
            solutionRangeMatrix[i][j].range[1] <== 9;
    	}
    }
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            // setting the input signal of the RangeProof matrices as the current value in the puzzle/solution matrices
            puzzleRangeMatrix[i][j].in <== puzzle[i][j];
            solutionRangeMatrix[i][j].in <== solution[i][j];

	    // asserting that both output signals of the puzzle and solution RangeProof matrices are 1 (i.e. true)
	    // which indicates that the input values are within the range.
	    // if one of the input values is NOT in the range, this assertion triggers and throws an error in the execution
	    assert(puzzleRangeMatrix[i][j].out && solutionRangeMatrix[i][j].out);
